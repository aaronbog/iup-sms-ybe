 ...
 */
  ...
  if (all || stats.ext_prop.triggered) {
  PRT ("external lemmas: %15" PRId64 "   %10.2f %%  of queries", stats.ext_prop.lemmas, percent (stats.ext_prop.lemmas, stats.ext_prop.triggered));
  PRT ("  final checks:  %15" PRId64 "   %10.2f %%  of queries", stats.ext_prop.final_check, percent (stats.ext_prop.final_check, stats.ext_prop.triggered));
  PRT ("  conflicting:   %15" PRId64 "   %10.2f %%  per learned", stats.ext_prop.conflicting, percent (stats.ext_prop.conflicting, stats.ext_prop.lemmas));
  PRT ("  propagating:   %15" PRId64 "   %10.2f %%  per learned", stats.ext_prop.propagated, percent (stats.ext_prop.propagated, stats.ext_prop.lemmas));
  }
